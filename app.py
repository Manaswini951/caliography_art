...
st.caption("Extracts hand-drawn lettering without paper noise, animates organic stroke drawing on pure white, and blooms into vibrant colors.")
...
def compute_geodesic_progress_map(comp_mask, direction="Top -> Bottom"):
    ...
    using geodesic distance propagation (Fast Marching / BFS on pixel grid).
    """
    ys, xs = np.where(comp_mask > 0)
    if len(xs) == 0:
        return np.zeros_like(comp_mask, dtype=np.float32), []

    # Choose the stroke origin point based on direction
    ...
    start_pt = tuple(points[start_idx])  # (y, x)
    ...
    # 8-connected BFS for smooth continuous geodesic front propagation
...
def render_frame(
    ...
    write_ratio=0.75, show_pen=True, glow_intensity=0.3
):
    ...
    for i, comp in enumerate(components):
        ...
        # Soft antialiasing for the leading brush edge
    ...
    # --- Phase 2: Color Glow & Final Merge ---
    if t_global > write_phase_end:
        t_glow = (t_global - write_phase_end) / (1.0 - write_phase_end)
        ...
        # Radial bloom around vibrant strokes
...
write_speed = st.sidebar.slider("Writing Phase Share (%)", 50, 90, 75, help="Portion of time spent drawing letters before color glow starts.")
total_frames = st.sidebar.slider("Total Frames", 30, 120, 60, step=5)
...
if uploaded_files:
    ...
    for tab, file in zip(tabs, uploaded_files):
        with tab:
            ...
            # Resize if overly large for fast fluid rendering
            max_size = 900
            h, w = img.shape[:2]
            if max(h, w) > max_size:
            ...
            components, clean_mask = clean_and_extract_strokes = clean_and_extract_elements(img)
            enhanced = enhance_vibrancy(img)

            c1, c2 = st.columns([1, 1])
            with c1:
                st.subheader("Cleaned Ink Isolation")
                st.image(cv2.cvtColor(clean_mask, cv2.COLOR_GRAY2RGB), caption="Extracted strokes (No paper borders)", use_container_width=True)
